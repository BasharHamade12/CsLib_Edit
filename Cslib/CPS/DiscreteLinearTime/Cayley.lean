...
lemma helper3 [FiniteDimensional ℂ σ]
  ...
  simp only [neg_smul]
  -- Now goal is: ↑a ^ n = ∑ x, -(p.coeff ↑x • ↑a ^ ↑x)
  -- Rewrite to: ↑a ^ n = -∑ x, p.coeff ↑x • ↑a ^ ↑x
...
lemma controllabilityColumnSpace_invariant [FiniteDimensional ℂ σ]
  ...
  -- Consider an arbitrary generator
  intro x hx

  -- x is in the union, so it's of the form (a^i)(B v) for some i < n and v
  simp only [Set.mem_iUnion] at hx
  obtain ⟨i, hx⟩ := hx
  simp only [Set.mem_range] at hx
  obtain ⟨v, rfl⟩ := hx

  -- We need to show a((a^i.val)(B v)) is in the controllability column space
  -- This equals (a^(i.val + 1))(B v)

  by_cases h : i.val + 1 < n
  · -- Case 1: i.val + 1 < n, so a^(i.val + 1) is already a generator
  ...
  · -- Case 2: i.val + 1 ≥ n, use Cayley-Hamilton
    push_neg at h

    -- By Cayley-Hamilton, the characteristic polynomial vanishes at a
    ...
    -- a^n can be expressed as a linear combination of lower powers
    -- This follows from Cayley-Hamilton: if p(a) = 0 where p is the char poly,
    -- then a^n = -(c₀I + c₁a + ... + c_{n-1}a^{n-1}) for some coefficients
    ...
    -- Now show that a((a^(n-1))(B v)) is in the span
    rw [i_eq]
    have : a.toLinearMap ((a ^ (n - 1)) (B v)) = (a ^ n) (B v) := by
      have hn : n = n - 1 + 1 := by omega  -- works because n > 0 from i : Fin n
